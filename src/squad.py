...
TOTAL_BUDGET_USD = 10000000.00  # 10 Million USD
...
def can_draft_player(conn, player_id):
    ...
    new_total_cost = current_total_cost + player['base_price_usd']
    if new_total_cost > TOTAL_BUDGET_USD:
        remaining = TOTAL_BUDGET_USD - current_total_cost
        return False, (f"Adding {player['name']} would exceed the total "
                      f"budget. Remaining budget: ${remaining:,.2f}")

    if player['nationality'] == 'Overseas':
        count = player_queries.get_drafted_overseas_players_count(conn)
        if count >= MAX_OVERSEAS_PLAYERS:
