...
class TestSquad(unittest.TestCase):
    ...
    def test_can_draft_player_squad_full(self):
        ...
        can_draft, reason = squad.can_draft_player(self.conn, 2) # Try to draft another
        self.assertFalse(can_draft)
        self.assertEqual(reason, "Maximum squad size reached.")

    def test_can_draft_player_overseas_full(self):
        """Test that an overseas player cannot be drafted if the overseas limit is reached."""
        ...
        can_draft, reason = squad.can_draft_player(self.conn, 6) # Try to draft Rashid Khan (Overseas)
        self.assertFalse(can_draft)
    ...
    def test_can_draft_player_budget_exceeded(self):
        ...
        can_draft, reason = squad.can_draft_player(self.conn, 2) # Try to draft Rohit (1.8M)
        self.assertFalse(can_draft)
    ...
    def test_get_squad_summary(self):
        ...
        self.assertEqual(summary_initial['remaining_budget'], squad.TOTAL_BUDGET_USD)

        player_queries.draft_player(self.conn, 1) # Draft Kohli (Indian, 2M)
        player_queries.draft_player(self.conn, 5) # Draft Maxwell (Overseas, 1.7M)
        ...
        self.assertEqual(summary_updated['remaining_budget'], squad.TOTAL_BUDGET_USD - 3700000.00)
